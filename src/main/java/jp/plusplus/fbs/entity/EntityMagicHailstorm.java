 ...
 */
public class EntityMagicHailstorm extends Entity implements IProjectile, IPacketThrower {
    public static final int STATE_FLOW=0;
    public static final int STATE_THROW=1;

    protected int state=STATE_FLOW;
    protected int shootTicks;

    /* フロー用 */
    protected float flowAngle;
    protected float flowRadius;
    ...
    public EntityMagicHailstorm(World p_i1582_1_) {
        super(p_i1582_1_);
    ...
    public EntityMagicHailstorm(World par1World, EntityLivingBase shooter, float flowRadius, float damage) {
        this(par1World);
        this.shootingEntity = shooter;

        //初期状態での向きの決定
        flowAngle=(float)(2*Math.PI*rand.nextFloat()-Math.PI);
        this.flowRadius=flowRadius;
        flowY=(float)(2*Math.PI*rand.nextFloat()-Math.PI);
        this.setLocationAndAngles(shooter.posX-flowRadius*MathHelper.sin(flowAngle), shooter.posY + (double) shooter.getEyeHeight()/2+0.25f*MathHelper.sin(flowAngle+flowY), shooter.posZ+flowRadius*MathHelper.cos(flowAngle), 0, 0);
        ...
        */
        this.motionX=this.motionY=this.motionZ=0.f;
        this.setThrowableHeading(this.motionX, this.motionY, this.motionZ, 0, 0);

        setDamage(damage);

        FBS.logger.info("sender:"+shootingEntity);
    }
    ...
    protected void readEntityFromNBT(NBTTagCompound nbt) {
        ...
        readFromPacketNBT(nbt.getCompoundTag("packetThrower"));
    }
    ...
    public boolean canExist(){
        if(shootingEntity==null) return false;
        if(state==STATE_FLOW) return shootingEntity instanceof EntityLivingBase?((EntityLivingBase)shootingEntity).isPotionActive(Registry.potionHailstorm):ticksExisted<20*5;
        return ticksExisted-shootTicks<20*5;
    }
    ...
    protected void spawnParticle(){
        ...
        //5.パーティクルつくる
        float r=0.6f+0.3f*rand.nextFloat();
        EntityTracksFX fx=new EntityTracksFX(worldObj, posX, posY, posZ, vector.xCoord, vector.yCoord, vector.zCoord, r, r, 1.0f, 1.0f);
        FMLClientHandler.instance().getClient().effectRenderer.addEffect(fx);

        if(state==STATE_THROW && (this.ticksExisted-shootTicks)%3==0) {
    ...
    public void onUpdate() {
        ...
        rotationYaw+=(float)(360/20.0f);
        rotationPitch-=(float)(360/40.0f);
        ...
        //パーティクルを生成
        if (FBS.proxy.getClientWorld() != null && worldObj.isRemote && this.ticksExisted % 3 == 0) {
            spawnParticle();
        }

        if (state == STATE_FLOW) {
            //フロー状態では常にshooter近辺にいる
            if(shootingEntity==null){
                setDead();
                return;
            }
            this.setLocationAndAngles(shootingEntity.posX - flowRadius * MathHelper.sin(flowAngle), shootingEntity.posY + (double) shootingEntity.getEyeHeight() / 2 + 0.25f * MathHelper.sin(flowAngle+flowY), shootingEntity.posZ + flowRadius * MathHelper.cos(flowAngle), rotationYaw, rotationPitch);
            this.setPosition(this.posX, this.posY, this.posZ);
            flowAngle += (float) (2 * Math.PI / 40.0f);

            // IMobを探して飛ぶ
            if (ticksExisted % 10 == 0) {
                int r = 10;
                AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(posX - r, posY - 2, posZ - r, posX+1 + r, posY + 4, posZ+1 + r);
                List list = worldObj.getEntitiesWithinAABB(IMob.class, aabb);
                if (!list.isEmpty()) {
                    state = STATE_THROW;
                    shootTicks=ticksExisted;
                    worldObj.playSoundAtEntity(this, "random.bow", 1.0F, 1.0F / (rand.nextFloat() * 0.4F + 1.2F) + 0.5F);

                    if(!worldObj.isRemote){
                        Entity mob = (Entity) list.get(rand.nextInt(list.size()));

                        //角度とか速度の設定
                        motionX = mob.posX - posX;
                        motionY = mob.posY + mob.getEyeHeight() - posY;
                        motionZ = mob.posZ - posZ;

                        float mag = MathHelper.sqrt_double(motionX * motionX + motionY * motionY + motionZ * motionZ);
                        motionX *= 1.25f / mag;
                        motionY *= 1.25f / mag;
                        motionZ *= 1.25f / mag;

                        PacketHandler.INSTANCE.sendToDimension(new MessagePacketThrower(this), worldObj.provider.dimensionId);
                    }
                }
            }
        } else if (state == STATE_THROW) {
            // 基本的に他の射出Entityと変わらない
    ...
    public void writeToPacketNBT(NBTTagCompound nbt) {
        nbt.setInteger("shooter", shootingEntity.getEntityId());
        nbt.setInteger("state", state);
        nbt.setInteger("shootTick", shootTicks);
        nbt.setFloat("angle", flowAngle);
        nbt.setFloat("radius", flowRadius);
        nbt.setFloat("y", flowY);
    ...
    public void readFromPacketNBT(NBTTagCompound nbt) {
        shootingEntity=worldObj.getEntityByID(nbt.getInteger("shooter"));
        state=nbt.getInteger("state");
        shootTicks=nbt.getInteger("shootTick");
        flowAngle=nbt.getFloat("angle");
        flowRadius=nbt.getFloat("radius");
        flowY=nbt.getFloat("y");

        motionX=nbt.getDouble("mx");
        motionY=nbt.getDouble("my");
